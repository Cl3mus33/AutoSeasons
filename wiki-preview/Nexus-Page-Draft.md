...
AutoSeasons also actively manages conflicts with other seasonal-content mods already in your load
order (Turn of the Seasons, Nature of the Wild Lands, etc.): it merges everyone's `Data/Seasons`
declarations into one authoritative ini per season, respects another mod's own coverage by
default, and lets you override specific mods - globally or per record type - or set an explicit
priority order between them via the built-in "Manage Season Mod Conflicts" dialog whenever two
mods compete for the same record.
